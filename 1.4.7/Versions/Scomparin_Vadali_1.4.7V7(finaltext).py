...
def algorithm_1():
    ...
    def watermark(original_image, alpha, image):
        ...
        final = PIL.Image.new("RGBA", (width + 50, height + 50), (101,171,174,150)) # creating a new image as the border that is 25 by 25 
    ...
    def alter_all_images(directory=None):
        ...
        for image in range(len(image_list)): #a for loop that iterates through the files
            ...
            new_image = watermark(image,0.4, blend_image) #calling the image watermark with its paramters
            new_image.paste(logo_small,(25,25),mask = logo_small)
            new_image.paste(donate_small, (1770,1062), mask = donate_small)
...
def algorithm_2():
    ...
    def alter_all_images(directory=None):
        ...
        if directory == None:
            directory = os.getcwd() # Use working directory if unspecified
        # Create a new directory 'modified'
        new_directory = os.path.join(directory, 'Algorithm 1.13')
        try:
            os.mkdir(new_directory)
        except OSError:
            pass # if the directory already exists, proceed
        ...
        image2.paste(logo_small, (0,0), mask = logo_small)
        image2.paste(donate_small, (400,1000), mask = donate_small)
        
        new_image = watermark(image2,0.5)
        
        new_image.save('final.png', format = "png")
...
def algorithm_3():
    '''Adds the border too all the images.
    '''
    ...
    def alter_all_images(rows,columns, directory=None):
        ...
        if directory == None:
            directory = os.getcwd() # Use working directory if unspecified
        # Create a new directory 'modified'
        new_directory = os.path.join(directory, 'Algorithm 3.8')
        try:
            os.mkdir(new_directory)
        except OSError:
            pass # if the directory already exists, proceed
...
def algorithm_4():
    ...
    def alter_all_images(directory=None):
        ...
        for image in range(len(image_list)):
            ...
            new_image.paste(logo_small,(25,25),mask = logo_small)
            new_image.paste(donate_small, (540,370), mask = donate_small)
            new_image_filename = os.path.join(new_directory, filename + '.png')
            new_image.save(new_image_filename) #saves img
